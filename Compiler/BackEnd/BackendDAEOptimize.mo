...
algorithm
  ...
  for syst in systs loop
    ...
    if not listEmpty(varLst) then
      ...
      // causalize again
      syst.orderedVars := BackendVariable.listVar1(MetaModelica.Dangerous.listReverseInPlace(varLstNew));
      syst.orderedEqs := BackendEquation.listEquation(MetaModelica.Dangerous.listReverseInPlace(eqLstNew));

...
public:
  ...
  virtual shared_ptr<ILinearAlgLoopSolver> createLinSolver(string solver_name, shared_ptr<ILinSolverSettings> solver_settings)
  {
    if(_last_selected_solver.compare(solver_name) == 0)
    {
            std::map<std::string, factory<ILinearAlgLoopSolver, ILinSolverSettings*> >::iterator iter;
            std::map<std::string, factory<ILinearAlgLoopSolver, ILinSolverSettings*> >& linSolverFactory(_linsolver_type_map->get());
            ...
            shared_ptr<ILinearAlgLoopSolver> solver = shared_ptr<ILinearAlgLoopSolver>(iter->second.create(solver_settings.get()));

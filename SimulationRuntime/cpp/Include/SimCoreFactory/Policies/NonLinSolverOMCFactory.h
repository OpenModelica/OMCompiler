...
public:
   ...
   virtual shared_ptr<INonLinearAlgLoopSolver> createNonLinSolver(string solver_name, shared_ptr<INonLinSolverSettings>  solver_settings)
   {
       if(_last_selected_solver.compare(solver_name)==0)
       {
            std::map<std::string, factory<INonLinearAlgLoopSolver, INonLinSolverSettings*> >::iterator iter;
            std::map<std::string, factory<INonLinearAlgLoopSolver, INonLinSolverSettings*> >& nonlinSolverFactory(_non_linsolver_type_map->get());
            ...
            shared_ptr<INonLinearAlgLoopSolver> solver = shared_ptr<INonLinearAlgLoopSolver>(iter->second.create(solver_settings.get()));
            return solver;
       }
       else
           throw ModelicaSimulationError(MODEL_FACTORY,"Selected nonlinear solver is not available");
